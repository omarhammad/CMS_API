...
function fillPage(patient) {
    ...
    document.getElementById('patient_name').innerText = patient['firstName'] + ' ' + patient['lastName'];
    ...
    if (old_appointments.length === 0) {
        ...
        for (const appointment of old_appointments) {
            ...
            const appointment_date = formatDate(new Date(appointment['appointmentDateTime']))
            const appointment_doctor = appointment.doctor['firstName'] + ' ' + appointment.doctor['lastName'];
            ...
            if (appointment_prescriptions === null) {
                ...
                medicationNames = medications.reduce((acc, medication, index, array) => {
